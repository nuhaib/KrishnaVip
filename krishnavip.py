...
# Source & Target Channels (Script 5)
source_channel_script5 = [-1002168406093]  # Replace with actual source channel(s) for Script 5
target_channels_script5 = [-1002428699939]  # Replace with actual target channels

# Initialize Telegram client
client = TelegramClient('script5_session', api_id, api_hash, flood_sleep_threshold=10)

@client.on(events.NewMessage(chats=source_channel_script5))
async def forward_messages(event):
    """Forward messages only to Script 5's assigned target channels."""
    msg = event.message
    ...
    tasks = []
    for channel_id in target_channels_script5:
...
async def main():
    ...
    print("Script 5 Forwarder is running...")
    await client.start()
